...
export const ReviewCard = ({ review, showActions }: ReviewCardProps) => (
    ...
    <div className="flex items-start justify-between">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-[11px] text-gray-400">{review.category}</p>
        </div>
      </div>
      <div className="flex items-center gap-4">
        ...
        <span className="text-[12px] text-gray-300">{review.date}</span>
      </div>
    </div>
      ...
      <div className="mb-2 flex items-center gap-1 text-orange-400">
        {[...Array(5)].map((_, i) => (
          <Star
            ...
            size={14}
            fill={
              i < Math.floor(review.rating) ? 'currentColor' : 'transparent'
            }
            className={i < Math.floor(review.rating) ? '' : 'text-gray-200'}
          />
        ))}
